    ...
    if (allImages && allImages.size() > 0) {
        for (var i = 0; i < allImages.size(); i++) {
            ...
            if (img) {
                ...
                if (img.id() == (targetPackageName +":id/fzc")) {
                    // 正确调用bounds()方法并点击
    ...
    if (allImages && allImages.size() > 0) {
        for (var i = 0; i < allImages.size(); i++) {
            ...
            if (img) {
                ...
                // 检查ID是否为"u3"
                if (img.id() == (targetPackageName +":id/u3")) {
                    // 正确调用bounds()方法并点击
                    if(i == 1){
...
function selectImageByButton(fileName) {
    ...
    try {
        ...
        if (allTextView && allTextView.size() > 0) {
            for (var i = 0; i < allTextView.size(); i++) {
                ...
                if (textView) {
                    ...
                    if (textView.id() == (targetPackageName +":id/tqg") || textView.id() == (targetPackageName +":id/tqj")) {
                        // 正确调用bounds()方法并点击
                        taskLog("找到顶部控件: 全部" );
        ...
        //点击对应的targetPath：A_NEST_Instagram_MEDIA
        ...
        if (allListTextView && allListTextView.size() > 0) {
            for (var i = 0; i < allListTextView.size(); i++) {
                ...
                if (listTextView) {
                    ...
                    // 检查text是否为"A_NEST_Instagram_MEDIA"
                    if (listTextView.text() == "A_NEST_Instagram_MEDIA") {
                        // 正确调用bounds()方法并点击
                        taskLog("找到对应目录" );
        ...
        if(targetPackageName == targetPackageName){
            autoSelectButton = id(targetPackageName + ":id/hj2").find();
        }else{
            autoSelectButton = id(targetPackageName + ":id/hkm").find();
        }
        ...
        for(var i = 0; i < autoSelectButton.size(); i++) {
            ...
            if(selectButton) {
                ...
                if(targetPackageName == targetPackageName){
                    clickId(targetPackageName + ":id/qxd")
                }else{
                    clickId(targetPackageName + ":id/r1r")
                }
                ...
                if(targetPackageName == targetPackageName){
                    clickId(targetPackageName + ":id/l4w")
                }else{
                    clickId(targetPackageName + ":id/l7b")
                }
                ...
                //删除临时媒体文件夹
                const delFolder = "/storage/emulated/0/Download/" + A_NEST_Instagram_MEDIA;  // 替换成你想要的文件夹路径
...
function click_choose_video_framelayout(){
    ...
    let view = id(targetPackageName + ":id/cg7").className("android.widget.FrameLayout").findOne();
